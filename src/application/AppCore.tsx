...
function AppCore(props: AppCoreProps): React.ReactElement {
  const { context } = props
  const [cookies, setCookie] = useCookies<HippoCookies, CookieValues>([HippoCookies.PAGE])
  const { user } = context
        ...
        <div className="breadcrumbs text-sm">
          <ul>
            ...
            {pages.map((page, index) => {
              ...
              return (
                <li key={page} onClick={() => state.setPage(PageOption.HOME)} className={'capitalize'}>
                  {page.toLowerCase()}
                </li>
              )
            })}
  ...
  return useObserver(() => (
    <div id={'hippo-app'} className={'w-full px-4'} data-theme={context.config.darkMode ? 'night' : 'bumblebee'}>
      <div
        id={'hippo-app-frame'}
      ...
      >
        <div className="navbar mt-2 mb-6 shadow-sm">
          <div className="navbar-start">
            <div className="dropdown">
              ...
              <ul tabIndex={0} className="menu menu-md dropdown-content rounded-box z-1 mt-3 w-52 p-2 shadow">
                  ...
                  return (
                    <li key={option}>
                      <a
                        ...
                        onClick={() => state.setPage(PageOption[option as keyof typeof PageOption])}
                      >
                        {label}
                      </a>
                    </li>
          ...
          <div className="navbar-center">
            <a className="btn btn-ghost px-6 py-8 hover:bg-default" onClick={() => state.setPage(PageOption.HOME)}>
              {state.brandDetails && (
                <div className={'align-center text-center w-fit mx-auto justify-content-center align-items-center'}>
                  <div className={'card-title text-primary'}>{state.brandDetails.name}</div>
                  <div className={'text-muted text-xs text-center ml-12'}>Site Management</div>
                </div>
              )}
            </a>
          </div>
        ...
        <main className={'flex-grow min-h-[95svh] w-full mx-auto px-2'}>
          ...
          {state.loading && <LoadingSection size={'lg'} message={'Initializing...'} />}
        </main>
