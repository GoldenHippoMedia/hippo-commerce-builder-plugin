...
function StatGridCard(props: StatGridCardProps): React.ReactElement {
  ...
  const cardClass = clsx(
    ...
    {
      ["bg-primary hover:bg-primary/70 text-primary-content shadow-primary"]:
        variant === "info",
      ["bg-success hover:bg-success/70 text-success-content shadow-success"]:
        variant === "success",
      ["bg-warning hover:bg-warning/70 text-warning-content shadow-warning"]:
        variant === "warning",
      ["bg-info hover:bg-info/70 text-info-content shadow-info"]:
        variant === "neutral",
      ["bg-error hover:bg-error/70 text-error-content shadow-error"]:
        variant === "error",
    },
    className,
  );
  ...
  return (
    <div className={cardClass}>
      <div className={"p-4"}>
        ...
        {!loading && (
          <div className={"text-6xl mx-auto ml-16 font-bold"}>
            {metricValue(metric, currency)}
          </div>
        )}

        {subtitle && (
          <div className={"text-sm ml-auto w-fit max-w-full"}>{subtitle}</div>
        )}
      </div>
      ...
      {actionLabel && onActionClick && (
        <div className={"px-4 pb-2"}>
          ...
          {loading && (
            <span className="mt-6 ml-4 loading loading-dots loading-md"></span>
          )}
        </div>
      )}
    </div>
