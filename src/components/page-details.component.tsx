...
import { PdpTypes } from "@core/models/page.model";
...
function PageDetailCard(props: Props) {
  ...
  const groupedValidationItems = React.useMemo(() => {
    const groups: Record<
      string,
      Array<{ original: string | any; details: any }>
    > = {
      SEO: [],
      General: [],
      Blog: [],
  ...
  const hasTrial =
    isProductPage &&
    (pdpInfo?.productDetails?.flavorsOrSizes ?? []).some((p) => p.trialSize);
  ...
  const groupProducts: IProduct[] =
    isProductPage && pdpInfo && pdpInfo.productDetails?.flavorsOrSizes
      ? pdpInfo.productDetails.flavorsOrSizes
          ...
          .filter(
            (productInfo): productInfo is IProduct => productInfo !== undefined,
          )
      : [];
  ...
  const singleProduct: IProduct | undefined =
    isProductPage && pdpInfo?.type === PdpTypes.PRODUCT
      ? Utils.getProductFromCatalog(
          pdpInfo.productDetails?.slug ?? "",
          products,
        )
      : nonTrialProduct
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="w-full p-2">
            ...
            {!page.thumbnail && (
              <div
                className={
                  "text-sm font-medium text-error wrap-break-word mr-2"
                }
              >
                No SEO Image found.
              </div>
            ...
            <div className="flex items-center gap-3 w-full">
              ...
              <div>
                ...
                {page.seoTitle && page.seoTitle.length > 0 && (
                  <div className="text-sm font-medium wrap-break-word mr-2">
                    {page.seoTitle}
                  </div>
                )}
                {(!page.seoTitle || page.seoTitle === "") && (
                  <div className="text-sm font-medium text-error mr-2">
                    No SEO Title Provided.
                  </div>
                )}
              </div>
            </div>
                ...
                {blogInfo.publicationDate && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-medium">
                        {formatDate(blogInfo.publicationDate)}
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {pdpInfo.subHeading && (
                    <div className="text-sm text-base-content/70">
                      <div
                        dangerouslySetInnerHTML={{ __html: pdpInfo.subHeading }}
                      />
                    </div>
                  )}
                </div>
                ...
                {pdpInfo.productDetails && (
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <StarRating
                        rating={pdpInfo.productDetails.reviews.average}
                        variant={"primary"}
                      />
                      <span className="text-sm font-medium">
                        {pdpInfo.productDetails.reviews.average.toFixed(2)}
                      </span>
                      <span className="text-sm text-base-content/60">
                        ({pdpInfo.productDetails.reviews.count.toLocaleString()}{" "}
                        reviews)
                      </span>
                    </div>
                  </div>
                ...
                {/* Categories */}
                {pdpInfo.productDetails?.categories &&
                  pdpInfo.productDetails.categories.length > 0 && (
                    <div>
                      <div className="text-xs text-base-content/60 mb-1">
                        Categories
                      </div>
                ...
                {/* Ingredients */}
                {pdpInfo.productDetails?.ingredients &&
                  pdpInfo.productDetails.ingredients.length > 0 && (
                    <div>
                      <div className="text-xs text-base-content/60 mb-1">
                        Ingredients
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {pdpInfo.productDetails.ingredients.map(
                          (ingredient) => (
                            <div
                              key={ingredient.id}
                ...
                {/* Use Cases */}
                {pdpInfo.productDetails?.useCases &&
                  pdpInfo.productDetails.useCases.length > 0 && (
                    <div>
                      <div className="text-xs text-base-content/60 mb-1">
                        Use Cases
                      </div>
                ...
                {/* Tags */}
                {pdpInfo.productDetails?.tags &&
                  pdpInfo.productDetails.tags.length > 0 && (
                    <div>
                      <div className="text-xs text-base-content/60 mb-1">
                        Tags
                      </div>
                  ...
                  <div className={"grid grid-cols-2 gap-2 lg:grid-cols-4"}>
                    <div>
                      ...
                      <div className="text-sm">
                        ${singleProduct.retailPrice.toFixed(2)}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-base-content/60 mb-1">
                    ...
                    <div>
                      ...
                      <div className="text-sm">
                        {singleProduct.packaging.singular} /{" "}
                        {singleProduct.packaging.plural}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-base-content/60 mb-1">
                      ...
                      <div className="text-sm">
                        {singleProduct.upc && singleProduct.upc.length > 0
                          ? singleProduct.upc
                          : "No UPC"}
                      </div>
                    </div>
                    ...
                    {singleProduct.restrictedCountries.length > 0 && (
                      <div className={"col-span-2"}>
                        ...
                        <div className="text-sm">
                          <ul>
                            {singleProduct.restrictedCountries.map(
                              (country) => (
            ...
            {pdpInfo.type === "Product Group" &&
              ...
              pdpInfo.productDetails.flavorsOrSizes.length > 0 && (
                <div className="mt-6">
                  ...
                  <div className={"text-xs italic ml-4 mt-1 mb-3"}>
                    Click to see pricing
                  </div>
                  <div className="grid gap-3">
                    {pdpInfo.productDetails.flavorsOrSizes.map(
                      (variant, index) => {
                        const productDetails = groupProducts.find(
                          (p) => p.slug === variant.slug,
                        );
                        return (
                          <div
                            key={variant.productId || index}
                            className="collapse bg-base-200 rounded-lg p-4"
                          >
                            <input type="checkbox" />
                            ...
                            <div className="collapse-content">
                              {productDetails && (
                                <ProductPricingTable product={productDetails} />
                              )}
                              {!productDetails && (
                                <p className={"text-sm text-error"}>
                                  Warning! No product found in the commerce
                                  feed.
                                </p>
                              )}
                            </div>
