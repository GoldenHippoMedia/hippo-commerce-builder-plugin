...
function PageList(props: PageListProps) {
    ...
    if (statusFilter !== 'all') {
      switch (statusFilter) {
        ...
        case 'warning':
          ...
          break
        case 'valid':
          filtered = filtered.filter((page) => page.issues.length === 0 && page.warnings.length === 0)
          break
        default:
          break
      }
  ...
  const stats = useMemo(() => {
    return {
      ...
      valid: filteredAndSortedPages.filter((p) => p.warnings.length === 0 && p.issues.length === 0).length,
      ...
      general: filteredAndSortedPages.filter((p) => p.pageType === 'General').length,
      product: filteredAndSortedPages.filter((p) => p.pageType === 'Product').length,
  ...
  const getFilterClass = (active: boolean) => {
    return clsx('flex align-items-center rounded-lg', active ? 'outline-primary ' : 'outline-neutral-content')
  }
  ...
  return (
    <section className={'p-4 max-w-7xl min-w-full mx-auto rounded-lg'}>
      ...
      <div className={'flex flex-wrap justify-between mb-2 px-4 md:px-12'}>
        <div>
          <div className={'text-sm uppercase tracking-tight'}>Filter by Status</div>
          <div className={'flex w-full flex-wrap gap-2 mt-2 mb-4'}>
            {(statusFilter === 'all' || statusFilter === 'valid') && stats.valid > 0 && (
              <div className={getFilterClass(statusFilter === 'valid')}>
                <button className={getButtonClass(statusFilter === 'valid')} onClick={() => handleStatusClick('valid')}>
                  <AiOutlineCheckCircle className={'text-success text-lg'} />
                  <div className={'text-xs'}>
                    <span className={'text-success'}>{stats.valid}</span> Valid Pages
                  </div>
                </button>
              </div>
            )}
            {(statusFilter === 'all' || statusFilter === 'warning') && stats.warning > 0 && (
              <div className={getFilterClass(statusFilter === 'warning')}>
                ...
                >
                  ...
                  <div className={'text-xs'}>
                    <span className={'text-warning'}>{stats.warning}</span> Pages with Warnings
                  </div>
                </button>
              </div>
            )}
            {(statusFilter === 'all' || statusFilter === 'invalid') && stats.invalid > 0 && (
              <div className={getFilterClass(statusFilter === 'invalid')}>
                ...
                >
                  ...
                  <div className={'text-xs'}>
                    <span className={'text-error'}>{stats.invalid}</span> Invalid Pages
                  </div>
                </button>
              </div>
            )}
        ...
        <div>
          <div className={'text-sm uppercase tracking-tight'}>Filter by Page Type</div>
          <div className={'flex w-full flex-wrap gap-2 mt-2 mb-4'}>
            {(filterType === 'All' || filterType === 'General') && stats.general > 0 && (
              <div className={getFilterClass(filterType === 'General')}>
                ...
                >
                  ...
                  <div className={'text-xs'}>
                    <span className={'text-info'}>{stats.general}</span> General Pages
                  </div>
                </button>
              </div>
            )}
            {(filterType === 'All' || filterType === 'Product') && stats.product > 0 && (
              <div className={getFilterClass(filterType === 'Product')}>
                ...
                >
                  ...
                  <div className={'text-xs'}>
                    <span className={'text-info'}>{stats.product}</span> Product Pages
                  </div>
                </button>
              </div>
            )}
            {(filterType === 'All' || filterType === 'Blog') && stats.blog > 0 && (
              <div className={getFilterClass(filterType === 'Blog')}>
                <button className={getButtonClass(filterType === 'Blog')} onClick={() => handleFilterClick('Blog')}>
                  <AiOutlineComment className={'text-info text-lg'} />
                  <div className={'text-xs'}>
                    <span className={'text-info'}>{stats.blog}</span> Blog Pages
                  </div>
                </button>
              </div>
            )}
      ...
      <div className={'flex gap-2 flex-wrap mb-2 items-center'}>
        ...
        <div
          className={'flex items-center cursor-pointer gap-2 p-2 rounded-lg hover:bg-base-200'}
      ...
      {filteredAndSortedPages.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4 pr-4">
          {filteredAndSortedPages.map((page) => (
            <PageCard key={page.id} page={page} onSelectPage={handlePageSelect} onSelectPageLabel="View Details" />
          ))}
        </div>
      )}

      {selectedPage && (
        <PageDetailsDialog page={selectedPage} onClose={() => handlePageSelect(undefined)} products={products} />
      )}
    </section>
  )
}
