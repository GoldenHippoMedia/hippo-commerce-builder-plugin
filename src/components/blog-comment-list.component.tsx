...
const BlogCommentList: React.FC<BlogCommentListProps> = ({
  ...
  const availableBlogs = useMemo(() => {
    const blogMap = new Map<string, { id: string; title: string; count: number }>()
    ...
    return Array.from(blogMap.values()).sort((a, b) => a.title.localeCompare(b.title))
  }, [comments])
  ...
  const filteredComments = useMemo(() => {
    ...
    if (selectedBlogId) {
      filtered = filtered.filter((comment) => comment.blogId === selectedBlogId)
    }
  ...
  const commentThreads = useMemo(() => {
    const buildThreads = (
      ...
      return filteredComments
        .filter((comment) => comment.parentId === parentId && !processed.has(comment.id))
        .sort((a, b) => {
  ...
  const paginatedThreads = commentThreads.slice((currentPage - 1) * pageSize, currentPage * pageSize)
  ...
  const getStatusCounts = () => {
    ...
    if (selectedBlogId) {
      baseComments = baseComments.filter((comment) => comment.blogId === selectedBlogId)
    }
    ...
    return {
      ...
      pending: baseComments.filter((c) => c.status === 'Pending Approval').length,
  ...
  const truncateHtml = (html: string, maxLength: number = 150): { content: string; isTruncated: boolean } => {
      ...
      <div key={comment.id} className="mb-4">
        <div
          className={`bg-base-100 border border-base-300 rounded-lg p-4 ${
            ...
            needsReply && statusFilter !== 'no-replies' ? 'ring-2 ring-warning/30' : ''
          }`}
        ...
        >
          ...
          <div className="mb-4">
            ...
            {isTruncated && (
              <button onClick={() => toggleExpanded(comment.id)} className="text-primary text-xs mt-1 hover:underline">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    ...
                    <span>
                      {threadStats.totalReplies} {threadStats.totalReplies === 1 ? 'reply' : 'replies'}
                    </span>
                  </div>
                ...
                <button onClick={() => toggleThreadExpanded(comment.id)} className="btn btn-ghost btn-sm gap-1">
                  {isThreadExpanded ? (
                    <>
                      Hide Replies
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {comment.status === 'Pending Approval' && (
                <>
                  <button onClick={() => onApprove(comment.id)} className="btn btn-success btn-sm gap-1">
                    <HiCheck className="h-4 w-4" />
                    Approve
                  </button>
                  <button onClick={() => onReject(comment.id)} className="btn btn-error btn-sm gap-1">
              ...
              {level < 5 && (
                <button onClick={() => onReply(comment.id, comment)} className="btn btn-outline btn-sm gap-1">
            ...
            <div className="text-xs text-base-content/40 font-mono">ID: {comment.id}</div>
          </div>
        </div>

        {/* Render replies only if thread is expanded */}
        {hasReplies && isThreadExpanded && (
          <div className="mt-4">{replies.map((replyThread) => renderComment(replyThread))}</div>
        )}
      </div>
    )
  }
    ...
    <div className={`bg-base-100 rounded-lg overflow-hidden ${className}`}>
      ...
      <div className="bg-base-200 p-4 border-b border-base-300">
        <div className="space-y-4">
          {/* Blog Selection */}
          <div>
            <label className="text-sm font-medium text-base-content/80 mb-2 block">Select Blog</label>
            <select
              ...
              {selectedBlogId && (
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-sm text-base-content/60">
                    Viewing: {availableBlogs.find((b) => b.id === selectedBlogId)?.title}
                  </span>
                  <button onClick={() => onViewBlog(selectedBlogId)} className="btn btn-ghost btn-xs gap-1">
      ...
      <div className="p-4">
        {paginatedThreads.length === 0 ? (
          <div>
            {loading && (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-base-content/50">Loading Comments...</p>
              </div>
            )}
            {!loading && (
              <div className="text-center py-12">
                <HiOutlineChatBubbleLeft className="h-12 w-12 text-base-content/30 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-base-content/60 mb-2">No Comments Found</h3>
          ...
          <div className="space-y-6">{paginatedThreads.map((thread) => renderComment(thread))}</div>
        )}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-8 pt-4 border-t border-base-300">
            <div className="text-sm text-base-content/60">
              Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, totalThreads)} of{' '}
              {totalThreads} comment threads
            </div>
